...
class RegressionModels_UI(QtWidgets.QWidget, Ui_Form):
    ...
    def ModelFitAndPredict(self):
        ...
        for ratio in train_set_ratio:
            ...
            y_train_pred = model.predict(train_x)  # Predict using training set
            y_test_pred = model.predict(test_x)  # Predict using test set
            #y_pred = model.predict(all_X)  # Predict using all data

            print('\n>>>>>> Result <<<<<< \n')
            print('Model: ' + model_name + '，Training set ratio: ' + str(ratio) + '，PCA mode: ' + str(self.comboBox_3.currentIndex()+1) + '\n')

            MAE = mean_absolute_error(test_y, y_test_pred)
            MSE = mean_squared_error(test_y, y_test_pred)
            RMSE = np.sqrt(mean_squared_error(test_y, y_test_pred))
            ...
            df3 = pd.DataFrame(columns=['Dataset row index', 'Predicted value', 'True value'])
            df3['Dataset row index'] = test_y.index.array + 2
            df3['Predicted value'] = y_test_pred
            df3['True value'] = test_y.values
            df3.to_excel(self.save_folder+'\{0} (Training set ratio {1}, PCA mode {2}, MAE {3}, RMSE {4}, R2 {5}) - Predicted and true values ​​of the test set.xlsx'.format(model_name, ratio, self.comboBox_3.currentIndex()+1, format(MAE, '.6f'), format(RMSE, '.6f'), format(R2, '.6f')), index=False)
            print('Save xlsx file: {0} (Training set ratio {1}, PCA mode {2}, MAE {3}, RMSE {4}, R2 {5}) - Predicted and true values ​​of the test set.xlsx'.format(model_name, ratio, self.comboBox_3.currentIndex()+1, format(MAE, '.6f'), format(RMSE, '.6f'), format(R2, '.6f')))

            ## Save the prediction results of all data to excel document
            #df4 = pd.DataFrame(columns=['Dataset row index', 'Predicted value', 'True value'])
            #df4['Dataset row index'] = all_Y.index.array + 2
            #df4['Predicted value'] = y_pred
            #df4['True value'] = all_Y.values
            #df4.to_excel(self.save_folder+'\{0} (Training set ratio {1}, PCA mode {2}) - Predicted and true values ​​of all data.xlsx'.format(model_name, ratio, self.comboBox_3.currentIndex()+1), index=False)
            #print('Save file: {0} (Training set ratio {1}, PCA mode {2}) - Predicted and true values ​​of all data.xlsx'.format(model_name, ratio, self.comboBox_3.currentIndex()+1))
    ...
    def PCA_mode_1(self, df, n_components = 4):
    ...
    def PCA_mode_2(self, df, n_components = 9):
